...
const BillPay: React.FC = () => {
  ...
  const handlePay = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('CLICK FIRED: bill_payment_init');
    setIsProcessing(true);
    pushToDataLayer('bill_payment_init', { category: selectedCat });
    
    setTimeout(() => {
        setIsProcessing(false);
        alert(`Bill Payment for ${selectedCat} successful!`);
        console.log('CLICK FIRED: bill_payment_complete');
        pushToDataLayer('bill_payment_complete', {
            ...
            <div className="md:col-span-1 space-y-2">
                {CATEGORIES.map((cat) => (
                    <button
                        key={cat.id}
                        onClick={() => {
                          console.log('CLICK FIRED: bill_category_select');
                          pushToDataLayer('bill_category_select', { category: cat.id });
