...
const ApplicationWizard: React.FC = () => {
  ...
  useEffect(() => {
    // Set page title based on product type
    document.title = `Apply for ${productType} - Nova Bank`;
    // Track funnel step on mount and step change
    console.log('CLICK FIRED: funnel_step');
  ...
  const handleNext = () => {
    console.log('CLICK FIRED: funnel_next');
    pushToDataLayer('funnel_next', {
      next_step: step + 1,
      product: productType
    });
    setStep(prev => prev + 1);
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-20 pb-12 px-4">
        <div className="max-w-xl mx-auto bg-white rounded-2xl shadow-lg p-8 text-center animate-in fade-in zoom-in duration-500">
          ...
          <Button onClick={() => {
                console.log('CLICK FIRED: back_to_home');
                pushToDataLayer('back_to_home');
                setTimeout(() => navigate('/'), 100);
              }} className="w-full">Back to Home</Button>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div className="bg-gray-50 px-8 py-4 border-t border-gray-100 flex justify-between items-center">
                <Button 
                    variant="ghost" 
                    onClick={() => {
                      console.log('CLICK FIRED: wizard_back');
                      pushToDataLayer('wizard_back', { current_step: step });
